...
Str show_ResourceState (ResourceState);
 ...
 // The Resource class refers to a resource with reference semantics.  The class
 //  itself is cheap to copy.
struct Resource {
 ...
 //  same name will have the same data pointer).
...
void load (Resource);
 ...
 //  will be cancelled and all of the given resources will be in the UNLOADED
...
void load (const std::vector<Resource>&);
 ...
 //  UNLOADED.  After renaming, old_res will be UNLOADED and new_res will be
 //  LOADED.
...
void save (const std::vector<Resource>&);
 ...
 //  nothing if the resource is UNLOADED, and throws if it is LOADING.  Scans
 //  all other LOADED resources to make sure none of them are referencing this
...
void unload (const std::vector<Resource>&);
 ...
 // Immediately unloads the file without scanning for references to it.  This
 //  is faster, but if there are any references to data in this resource, they
...
void reload (const std::vector<Resource>&);
 ...
 //  if another error occurs (permission denied, etc.)
 // Calling load on the resource should fail after this.
...
///// RESOURCE NAME MANAGEMENT
 ...
 //  changed if any file resources are LOADED or LOADING.
 // The given filename is interpreted as a directory; a trailing slash will be
 //  ignored.  Although resource names are validated before being combined with
 //  this filename, this filename itself will not be processed in any way.
...
void set_file_resource_root_from_exe (char* argv0);
 ...
 // If given name is absolute, returns it unchanged.  If it's relative, makes
 //  it absolute by considering it relative to base.  If base is not given, uses
 //  current_resource() as the base, and if current_resource() is not defined,
 //  throws X::UnresolvedResourceName.  This also replaces // and /./ with /,
 //  and collapses a/b/../c to a/c.  If the resulting name starts with /..,
 //  will throw X::ResourceNameOutsideRoot.  If name (or base) has invalid
 //  characters in it, throws X::InvalidResourceName.  If name is empty, returns
 //  base (or current_resource()).
...
struct ResourceHandler {
     ...
     //  and if it does, load it as an image object.  If you're daring, you can
     //  also read the file and check its magic number, though of course that
     //  won't work if the file doesn't exist yet.
    virtual bool ResourceHandler_can_handle (Resource) = 0;
     ...
     //  X::ResourceHandlerConflict wll be thrown.
    virtual double ResourceHandler_priority () { return 0; }

     // Will be called in load() and reload() to construct the value of the
     //  resource.
     // The resource's state will be LOAD_CONSTRUCTING.
     // The default implementation throws X::ResourceHandlerCantLoad
    virtual void ResourceHandler_load (Resource);
     ...
     //  then return a function that writes that string to a file.
     // The resource's state will be SAVE_VERIFYING during this call, and
     //  SAVE_COMMITTING during the callback.
     // The default implementation throws X::ResourceHandlerCantSave
    virtual std::function<void()> ResourceHandler_save (Resource);
     // Will be called in remove_source() to delete the source.
     // The default implementation throws X::ResourceHandlerCantRemoveSource
    virtual void ResourceHandler_remove_source (Resource);
     // Will be called in rename() after moving the vlaue from the old resource
     //  to the new one.  from will be UNLOADED and to will be LOADED.  It's not
     //  recommended to do anything in this beside update filenames or Resource
     //  objects.
     // from's state will be UNLOADED and to's state will be LOADED.
    ...
    ~ResourceHandler() {
     ...
     //  shouldn't need have to call them yourself.
    void activate ();
    void deactivate ();
};
